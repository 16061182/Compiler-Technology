...
#------------34  FACTOR_FUNC INT getint save in reg[12]
   sw $v0 -48($sp)

#------------35  FACTOR_VAR INT i save in reg[13]
   lw $t3 0($sp)
   sw $t3 -52($sp)

#------------36  FACTOR_CON INT 2 save in reg[14]
   li $t0 2
   sw $t0 -56($sp)

#------------37  reg[14] = reg[13] JIAN reg[14]
   lw $t0 -52($sp)
   lw $t1 -56($sp)
   sub $t2 $t0 $t1
   sw $t2 -56($sp)

#------------38  PUSH INT parareg[1] from reg[14]
   sw $ra -60($sp)
   lw $t0 -56($sp)
   sw $t0 -64($sp)
...
#------------41  reg[15] = reg[12] JIA reg[15]
   lw $t0 -48($sp)
   lw $t1 -60($sp)
   add $t2 $t0 $t1
   sw $t2 -60($sp)

#------------42  RETURN reg[15] TO $V0 AND THEN GOTO 0
   lw $v0 -60($sp)
   jr $ra
...
#------------271  FACTOR_FUNC INT getint save in reg[24]
   sw $v0 -252($sp)

#------------272  FACTOR_CON INT 1 save in reg[25]
   li $t0 1
   sw $t0 -256($sp)

#------------273  FACTOR_ARRAY INT lmn [reg[25]] save in reg[26]
   lw $t3 -256($sp)
   sll $t3 $t3 2
   sub $t3 $sp $t3
   lw $t3 -56($t3)
...
#------------274  PUSH INT parareg[3] from reg[26]
   sw $ra -264($sp)
   lw $t0 -260($sp)
   sw $t0 -268($sp)
...
#------------276  FACTOR_FUNC INT getint save in reg[27]
   sw $v0 -264($sp)

#------------277  reg[27] = reg[24] JIA reg[27]
   lw $t0 -252($sp)
   lw $t1 -264($sp)
   add $t2 $t0 $t1
   sw $t2 -264($sp)

#------------278  FACTOR_CON INT 2 save in reg[28]
   li $t0 2
   sw $t0 -268($sp)

#------------279  FACTOR_ARRAY INT lmn [reg[28]] save in reg[29]
   lw $t3 -268($sp)
   sll $t3 $t3 2
   sub $t3 $sp $t3
   lw $t3 -56($t3)
...
#------------280  PUSH INT parareg[4] from reg[29]
   sw $ra -276($sp)
   lw $t0 -272($sp)
   sw $t0 -280($sp)
...
#------------282  FACTOR_FUNC INT getint save in reg[30]
   sw $v0 -276($sp)

#------------283  reg[30] = reg[27] JIA reg[30]
   lw $t0 -264($sp)
   lw $t1 -276($sp)
   add $t2 $t0 $t1
   sw $t2 -276($sp)

#------------284  WRITE INT reg[30]
   li $v0 1
   lw $a0 -276($sp)
   syscall
   li $v0 11
...
#------------286  FACTOR_VAR INT abcd save in reg[31]
   lw $t3 0($sp)
   sw $t3 -280($sp)

#------------287  PUSH INT parareg[5] from reg[31]
   sw $ra -284($sp)
   lw $t0 -280($sp)
   sw $t0 -288($sp)
...
#------------289  FACTOR_FUNC INT getint save in reg[32]
   sw $v0 -284($sp)

#------------290  WRITE INT reg[32]
   li $v0 1
   lw $a0 -284($sp)
   syscall
   li $v0 11
...
#------------292  FACTOR_VAR INT hijk save in reg[33]
   lw $t3 -52($sp)
   sw $t3 -288($sp)

#------------293  FACTOR_VAR INT o save in reg[34]
   lw $t3 -76($sp)
   sw $t3 -292($sp)

#------------294  FACTOR_VAR CHAR cha1 save in reg[35]
   lw $t3 -4($sp)
   sw $t3 -296($sp)

#------------295  PUSH INT parareg[6] from reg[33]
   sw $ra -300($sp)
   lw $t0 -288($sp)
   sw $t0 -304($sp)

#------------296  PUSH INT parareg[7] from reg[34]
   lw $t0 -292($sp)
   sw $t0 -308($sp)

#------------297  PUSH INT parareg[8] from reg[35]
   lw $t0 -296($sp)
   sw $t0 -312($sp)
...
#------------299  FACTOR_VAR INT o save in reg[36]
   lw $t3 -76($sp)
   sw $t3 -300($sp)

#------------300  FACTOR_VAR INT hijk save in reg[37]
   lw $t3 -52($sp)
   sw $t3 -304($sp)

#------------301  FACTOR_VAR CHAR cha1 save in reg[38]
   lw $t3 -4($sp)
   sw $t3 -308($sp)

#------------302  PUSH INT parareg[9] from reg[36]
   sw $ra -312($sp)
   lw $t0 -300($sp)
   sw $t0 -316($sp)

#------------303  PUSH INT parareg[10] from reg[37]
   lw $t0 -304($sp)
   sw $t0 -320($sp)

#------------304  PUSH INT parareg[11] from reg[38]
   lw $t0 -308($sp)
   sw $t0 -324($sp)
...
#------------306  FACTOR_VAR INT hijk save in reg[39]
   lw $t3 -52($sp)
   sw $t3 -312($sp)

#------------307  FACTOR_VAR INT hijk save in reg[40]
   lw $t3 -52($sp)
   sw $t3 -316($sp)

#------------308  FACTOR_VAR CHAR cha2 save in reg[41]
   lw $t3 -8($sp)
   sw $t3 -320($sp)

#------------309  PUSH INT parareg[12] from reg[39]
   sw $ra -324($sp)
   lw $t0 -312($sp)
   sw $t0 -328($sp)

#------------310  PUSH INT parareg[13] from reg[40]
   lw $t0 -316($sp)
   sw $t0 -332($sp)

#------------311  PUSH INT parareg[14] from reg[41]
   lw $t0 -320($sp)
   sw $t0 -336($sp)
...
#------------314  FACTOR_VAR_EXTERN INT b save in reg[42]
   lw $t3 .var_b
   sw $t3 -324($sp)

#------------315  WRITE INT reg[42]
   li $v0 1
   lw $a0 -324($sp)
   syscall
   li $v0 11
...
#------------316  FACTOR_CON INT 7 save in reg[43]
   li $t0 7
   sw $t0 -328($sp)

#------------317  FACTOR_ARRAY_EXTERN INT add [reg[43]] save in reg[44]
   lw $t3 -328($sp)
   sll $t3 $t3 2
   lw $t3 .var_add($t3)
   sw $t3 -332($sp)

#------------318  FACTOR_CON INT 8 save in reg[45]
   li $t0 8
   sw $t0 -336($sp)

#------------319  FACTOR_ARRAY_EXTERN INT add [reg[45]] save in reg[46]
   lw $t3 -336($sp)
   sll $t3 $t3 2
   lw $t3 .var_add($t3)
   sw $t3 -340($sp)

#------------320  reg[46] = reg[44] JIA reg[46]
   lw $t0 -332($sp)
   lw $t1 -340($sp)
   add $t2 $t0 $t1
   sw $t2 -340($sp)

#------------321  FACTOR_CON INT 9 save in reg[47]
   li $t0 9
   sw $t0 -344($sp)

#------------322  FACTOR_ARRAY_EXTERN INT add [reg[47]] save in reg[48]
   lw $t3 -344($sp)
   sll $t3 $t3 2
   lw $t3 .var_add($t3)
   sw $t3 -348($sp)

#------------323  reg[48] = reg[46] JIA reg[48]
   lw $t0 -340($sp)
   lw $t1 -348($sp)
   add $t2 $t0 $t1
   sw $t2 -348($sp)

#------------324  WRITE INT reg[48]
   li $v0 1
   lw $a0 -348($sp)
   syscall
   li $v0 11
...
#------------325  FACTOR_CON CHAR 99 save in reg[49]
   li $t0 99
   sw $t0 -352($sp)

#------------326  WRITE CHAR reg[49]
   li $v0 11
   lw $a0 -352($sp)
   syscall
   li $v0 11
...
#------------327  FACTOR_VAR CHAR cha1 save in reg[50]
   lw $t3 -4($sp)
   sw $t3 -356($sp)

#------------328  WRITE CHAR reg[50]
   li $v0 11
   lw $a0 -356($sp)
   syscall
   li $v0 11
...
#------------329  FACTOR_VAR CHAR cha1 save in reg[51]
   lw $t3 -4($sp)
   sw $t3 -360($sp)

#------------330  FACTOR_VAR CHAR cha2 save in reg[52]
   lw $t3 -8($sp)
   sw $t3 -364($sp)

#------------331  reg[52] = reg[51] JIA reg[52]
   lw $t0 -360($sp)
   lw $t1 -364($sp)
   add $t2 $t0 $t1
   sw $t2 -364($sp)

#------------332  WRITE INT reg[52]
   li $v0 1
   lw $a0 -364($sp)
   syscall
   li $v0 11
...
#------------333  FACTOR_CON INT 0 save in reg[53]
   li $t0 0
   sw $t0 -368($sp)

#------------334  FACTOR_VAR CHAR cha1 save in reg[54]
   lw $t3 -4($sp)
   sw $t3 -372($sp)

#------------335  ASSIGN_ARR CHAR efg[reg[53]] = reg[54]
   lw $t0 -368($sp)
   lw $t1 -372($sp)
   sll $t0 $t0 2
   sub $t0 $sp $t0
...
#------------336  FACTOR_CON INT 0 save in reg[55]
   li $t0 0
   sw $t0 -376($sp)

#------------337  FACTOR_ARRAY CHAR efg [reg[55]] save in reg[56]
   lw $t3 -376($sp)
   sll $t3 $t3 2
   sub $t3 $sp $t3
   lw $t3 -12($t3)
