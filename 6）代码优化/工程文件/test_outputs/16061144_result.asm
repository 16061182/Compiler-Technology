...
#------------107  FACTOR_FUNC INT gcd save in reg[14]
   sw $v0 -60($sp)

#------------108  RETURN reg[14] TO $V0 AND THEN GOTO 0
   lw $v0 -60($sp)
   jr $ra
...
#------------234  FACTOR_FUNC CHAR f2 save in reg[13]
   sw $v0 -68($sp)

#------------235  ASSIGN CHAR q = reg[13]
   lw $t3 -68($sp)
   sw $t3 0($sp)
...
#------------239  FACTOR_VAR INT w save in reg[14]
   lw $t3 -4($sp)
   sw $t3 -72($sp)

#------------240  FACTOR_VAR INT e save in reg[15]
   lw $t3 -8($sp)
   sw $t3 -76($sp)

#------------241  PUSH INT parareg[2] from reg[14]
   sw $ra -80($sp)
   lw $t0 -72($sp)
   sw $t0 -84($sp)

#------------242  PUSH INT parareg[3] from reg[15]
   lw $t0 -76($sp)
   sw $t0 -88($sp)
...
#------------244  FACTOR_FUNC INT gcd save in reg[16]
   sw $v0 -80($sp)

#------------245  ASSIGN INT r = reg[16]
   lw $t3 -80($sp)
   sw $t3 -12($sp)
...
#------------247  FACTOR_VAR INT r save in reg[17]
   lw $t3 -12($sp)
   sw $t3 -84($sp)

#------------248  WRITE INT reg[17]
   li $v0 1
   lw $a0 -84($sp)
   syscall
   li $v0 11
...
#------------250  FACTOR_VAR_EXTERN CHAR iftest save in reg[18]
   lw $t3 .var_iftest
   sw $t3 -88($sp)

#------------251  PUSH INT parareg[4] from reg[18]
   sw $ra -92($sp)
   lw $t0 -88($sp)
   sw $t0 -96($sp)
...
#------------253  FACTOR_CON CHAR 95 save in reg[19]
   li $t0 95
   sw $t0 -92($sp)

#------------254  WRITE CHAR reg[19]
   li $v0 11
   lw $a0 -92($sp)
   syscall
   li $v0 11
...
#------------255  FACTOR_CON CHAR 48 save in reg[20]
   li $t0 48
   sw $t0 -96($sp)

#------------256  WRITE CHAR reg[20]
   li $v0 11
   lw $a0 -96($sp)
   syscall
   li $v0 11
...
#------------257  FACTOR_CON INT 1 save in reg[21]
   li $t0 1
   sw $t0 -100($sp)

#------------258  ASSIGN INT nothing = reg[21]
   lw $t3 -100($sp)
   sw $t3 -16($sp)

#------------259  FACTOR_VAR INT nothing save in reg[22]
   lw $t3 -16($sp)
   sw $t3 -104($sp)

#------------260  IF reg[22] BEQ reg[0] THEN GOTO label_21
   lw $t0 -104($sp)
    beq $t0 $0 label_21
...
#------------264  FACTOR_CON INT 4 save in reg[23]
   li $t0 4
   sw $t0 -108($sp)

#------------265  FACTOR_CON INT 123456 save in reg[24]
   li $t0 123456
   sw $t0 -112($sp)

#------------266  ASSIGN_ARR_EXTERN INT g2[reg[23]] = reg[24]
   lw $t0 -108($sp)
   lw $t1 -112($sp)
   sll $t0 $t0 2
   sw $t1 .var_g2($t0)

#------------267  FACTOR_CON INT 3 save in reg[25]
   li $t0 3
   sw $t0 -116($sp)

#------------268  FACTOR_CON INT 7890 save in reg[26]
   li $t0 7890
   sw $t0 -120($sp)

#------------269  ASSIGN_ARR_EXTERN INT g2[reg[25]] = reg[26]
   lw $t0 -116($sp)
   lw $t1 -120($sp)
   sll $t0 $t0 2
   sw $t1 .var_g2($t0)

#------------270  FACTOR_CON INT 2 save in reg[27]
   li $t0 2
   sw $t0 -124($sp)

#------------271  FACTOR_CON INT 2 save in reg[28]
   li $t0 2
   sw $t0 -128($sp)

#------------272  reg[28] = reg[27] JIA reg[28]
   lw $t0 -124($sp)
   lw $t1 -128($sp)
   add $t2 $t0 $t1
   sw $t2 -128($sp)

#------------273  FACTOR_ARRAY_EXTERN INT g2 [reg[28]] save in reg[29]
   lw $t3 -128($sp)
   sll $t3 $t3 2
   lw $t3 .var_g2($t3)
   sw $t3 -132($sp)

#------------274  FACTOR_CON INT 4 save in reg[30]
   li $t0 4
   sw $t0 -136($sp)

#------------275  FACTOR_CON INT 1 save in reg[31]
   li $t0 1
   sw $t0 -140($sp)

#------------276  reg[31] = reg[30] JIAN reg[31]
   lw $t0 -136($sp)
   lw $t1 -140($sp)
   sub $t2 $t0 $t1
   sw $t2 -140($sp)

#------------277  FACTOR_ARRAY_EXTERN INT g2 [reg[31]] save in reg[32]
   lw $t3 -140($sp)
   sll $t3 $t3 2
   lw $t3 .var_g2($t3)
   sw $t3 -144($sp)

#------------278  reg[32] = reg[29] JIA reg[32]
   lw $t0 -132($sp)
   lw $t1 -144($sp)
   add $t2 $t0 $t1
   sw $t2 -144($sp)

#------------279  WRITE INT reg[32]
   li $v0 1
   lw $a0 -144($sp)
   syscall
   li $v0 11
...
#------------282  FACTOR_VAR INT r save in reg[33]
   lw $t3 -12($sp)
   sw $t3 -148($sp)

#------------283  FACTOR_VAR CHAR q save in reg[34]
   lw $t3 0($sp)
   sw $t3 -152($sp)

#------------284  PUSH INT parareg[5] from reg[33]
   sw $ra -156($sp)
   lw $t0 -148($sp)
   sw $t0 -160($sp)

#------------285  PUSH INT parareg[6] from reg[34]
   lw $t0 -152($sp)
   sw $t0 -164($sp)
