...
void printmidcode(){
    int i;
    for(i=0;i<midtab.index;i++){//打印四元式表内容
        //cout << "   kind:" << kinds[midtab.midcodes[i].kind] << " type:" << types[midtab.midcodes[i].type] << " name1:" << midtab.midcodes[i].name1 << " name2:" << midtab.midcodes[i].name2 << " t1:" << midtab.midcodes[i].t1 << " t2:" << midtab.midcodes[i].t2 <<" value:" << midtab.midcodes[i].value << endl;
        ...
        if(kind == CONST){
            ...
            cout << i << "  " << "IF " << "reg[" << t1 << "] " << kinds[temp.kind] << " reg[" << t2 << "] THEN GOTO " << value << endl;
        }
        else if(kind == JUMP){
            cout << i << "  " << "JUMP to " << value << endl;
        }
        else if(kind == READ){
