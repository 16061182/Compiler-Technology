...
#------------84  FACTOR_VAR INT var1 save in reg[22]
   lw $t3 1($sp)
   sw $t3 -88($sp)
...
#------------182  FACTOR_FUNC INT fib save in reg[8]
   sw $v0 -32($sp)

#------------183  FACTOR_VAR INT a save in reg[9]
   lw $t3 0($sp)
   sw $t3 -36($sp)

#------------184  FACTOR_CON INT 2 save in reg[10]
   li $t0 2
   sw $t0 -40($sp)

#------------185  reg[10] = reg[9] JIAN reg[10]
   lw $t0 -36($sp)
   lw $t1 -40($sp)
   sub $t2 $t0 $t1
   sw $t2 -40($sp)

#------------186  PUSH INT parareg[1] from reg[10]
   sw $ra -44($sp)
   lw $t0 -40($sp)
   sw $t0 -48($sp)
...
#------------188  FACTOR_FUNC INT fib save in reg[10]
   sw $v0 -40($sp)

#------------189  reg[10] = reg[8] JIA reg[10]
   lw $t0 -32($sp)
   lw $t1 -40($sp)
   add $t2 $t0 $t1
   sw $t2 -40($sp)

#------------190  RETURN reg[10] TO $V0 AND THEN GOTO 0
   lw $v0 -40($sp)
   jr $ra
...
#------------307  FACTOR_FUNC INT add save in reg[6]
   sw $v0 -28($sp)

#------------308  ASSIGN INT result = reg[6]
   lw $t3 -28($sp)
   sw $t3 0($sp)
...
#------------310  FACTOR_FUNC INT exptest save in reg[6]
   sw $v0 -28($sp)

#------------311  FACTOR_VAR INT result save in reg[7]
   lw $t3 0($sp)
   sw $t3 -32($sp)

#------------312  reg[7] = reg[6] JIA reg[7]
   lw $t0 -28($sp)
   lw $t1 -32($sp)
   add $t2 $t0 $t1
   sw $t2 -32($sp)
...
#------------314  FACTOR_FUNC INT arraytest save in reg[7]
   sw $v0 -32($sp)

#------------315  reg[7] = reg[7] JIA reg[7]
   lw $t0 -32($sp)
   lw $t1 -32($sp)
   add $t2 $t0 $t1
   sw $t2 -32($sp)
...
#------------318  FACTOR_VAR INT fibnum save in reg[8]
   lw $t3 -4($sp)
   sw $t3 -36($sp)

#------------319  FACTOR_CON INT 10 save in reg[9]
   li $t0 10
   sw $t0 -40($sp)

#------------320  IF reg[8] BLEQ reg[9] THEN GOTO label_31
   lw $t0 -36($sp)
   lw $t1 -40($sp)
    ble $t0 $t1 label_31
...
#------------324  FACTOR_VAR INT fibnum save in reg[10]
   lw $t3 -4($sp)
   sw $t3 -44($sp)

#------------325  FACTOR_CON INT 1 save in reg[11]
   li $t0 1
   sw $t0 -48($sp)

#------------326  IF reg[10] BGEQ reg[11] THEN GOTO label_32
   lw $t0 -44($sp)
   lw $t1 -48($sp)
    bge $t0 $t1 label_32
...
#------------331  FACTOR_VAR INT fibnum save in reg[12]
   lw $t3 -4($sp)
   sw $t3 -52($sp)

#------------332  PUSH INT parareg[8] from reg[12]
   sw $ra -56($sp)
   lw $t0 -52($sp)
   sw $t0 -60($sp)
...
#------------334  FACTOR_FUNC INT fib save in reg[12]
   sw $v0 -52($sp)
...
#------------338  FACTOR_CON INT 10 save in reg[13]
   li $t0 10
   sw $t0 -56($sp)

#------------339  PUSH INT parareg[9] from reg[13]
   sw $ra -60($sp)
   lw $t0 -56($sp)
   sw $t0 -64($sp)
...
#------------343  FACTOR_CON INT 314 save in reg[14]
   li $t0 314
   sw $t0 -60($sp)

#------------344  FACTOR_CON INT 314 save in reg[15]
   li $t0 314
   sw $t0 -64($sp)

#------------345  reg[15] = reg[14] JIAN reg[15]
   lw $t0 -60($sp)
   lw $t1 -64($sp)
   sub $t2 $t0 $t1
   sw $t2 -64($sp)

#------------346  FACTOR_CON INT 0 save in reg[16]
   li $t0 0
   sw $t0 -68($sp)

#------------347  FACTOR_ARRAY_EXTERN INT varray [reg[16]] save in reg[17]
   lw $t3 -68($sp)
   sll $t3 $t3 2
   lw $t3 .var_varray($t3)
   sw $t3 -72($sp)

#------------348  reg[17] = reg[15] JIA reg[17]
   lw $t0 -64($sp)
   lw $t1 -72($sp)
   add $t2 $t0 $t1
   sw $t2 -72($sp)

#------------349  FACTOR_CON INT 1 save in reg[18]
   li $t0 1
   sw $t0 -76($sp)

#------------350  reg[18] = reg[17] JIAN reg[18]
   lw $t0 -72($sp)
   lw $t1 -76($sp)
   sub $t2 $t0 $t1
   sw $t2 -76($sp)

#------------351  IF reg[18] BEQ reg[0] THEN GOTO label_35
   lw $t0 -76($sp)
    beq $t0 $0 label_35
...
#------------356  FACTOR_CON INT 1 save in reg[19]
   li $t0 1
   sw $t0 -80($sp)

#------------357  IF reg[19] BEQ reg[0] THEN GOTO label_39
   lw $t0 -80($sp)
    beq $t0 $0 label_39

#------------358  FACTOR_VAR_EXTERN CHAR var4 save in reg[20]
   lw $t3 .var_var4
   sw $t3 -84($sp)

#------------359  FACTOR_CON INT 0 save in reg[21]
   li $t0 0
   sw $t0 -88($sp)

#------------360  reg[21] = reg[20] JIA reg[21]
   lw $t0 -84($sp)
   lw $t1 -88($sp)
   add $t2 $t0 $t1
   sw $t2 -88($sp)

#------------361  PUSH INT parareg[10] from reg[21]
   sw $ra -92($sp)
   lw $t0 -88($sp)
   sw $t0 -96($sp)
...
#------------363  FACTOR_FUNC CHAR chara save in reg[21]
   sw $v0 -88($sp)

#------------364  FACTOR_CON CHAR 97 save in reg[22]
   li $t0 97
   sw $t0 -92($sp)

#------------365  IF reg[21] BNE reg[22] THEN GOTO label_37
   lw $t0 -88($sp)
   lw $t1 -92($sp)
    bne $t0 $t1 label_37
...
#------------376  FACTOR_FUNC CHAR chartest save in reg[22]
   sw $v0 -92($sp)

#------------377  WRITE CHAR reg[22]
   li $v0 11
   lw $a0 -92($sp)
   syscall
   li $v0 11
