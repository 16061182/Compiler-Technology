...
#------------370  FACTOR_CON INT 0 save in reg[50]
   li $t0 0
   sw $t0 -204($sp)

#------------371  FACTOR_CON CHAR 70 save in reg[51]
   li $t0 70
   sw $t0 -208($sp)

#------------372  ASSIGN_ARR_EXTERN CHAR testresult[reg[50]] = reg[51]
   lw $t0 -204($sp)
   lw $t1 -208($sp)
   sll $t0 $t0 2
   sw $t1 .var_testresult($t0)
...
#------------375  FACTOR_FUNC INT testwhile save in reg[52]
   sw $v0 -212($sp)

#------------376  IF reg[52] BEQ reg[0] THEN GOTO label_33
   lw $t0 -212($sp)
    beq $t0 $0 label_33

#------------377  FACTOR_CON INT 1 save in reg[53]
   li $t0 1
   sw $t0 -216($sp)

#------------378  FACTOR_CON CHAR 84 save in reg[54]
   li $t0 84
   sw $t0 -220($sp)

#------------379  ASSIGN_ARR_EXTERN CHAR testresult[reg[53]] = reg[54]
   lw $t0 -216($sp)
   lw $t1 -220($sp)
   sll $t0 $t0 2
   sw $t1 .var_testresult($t0)

#------------380  JUMP to label_34
   j label_34

#------------381  label_33    ----------------Label----------------
label_33:

#------------382  FACTOR_CON INT 1 save in reg[55]
   li $t0 1
   sw $t0 -224($sp)

#------------383  FACTOR_CON CHAR 70 save in reg[56]
   li $t0 70
   sw $t0 -228($sp)

#------------384  ASSIGN_ARR_EXTERN CHAR testresult[reg[55]] = reg[56]
   lw $t0 -224($sp)
   lw $t1 -228($sp)
   sll $t0 $t0 2
   sw $t1 .var_testresult($t0)
...
#------------387  FACTOR_FUNC CHAR testfactorial save in reg[57]
   sw $v0 -232($sp)

#------------388  FACTOR_EXPR reg[57] save in reg[58]
   lw $t0 -232($sp)
   sw $t0 -236($sp)

#------------389  FACTOR_CON CHAR 84 save in reg[59]
   li $t0 84
   sw $t0 -240($sp)

#------------390  IF reg[58] BNE reg[59] THEN GOTO label_35
   lw $t0 -236($sp)
   lw $t1 -240($sp)
    bne $t0 $t1 label_35

#------------391  FACTOR_CON INT 2 save in reg[60]
   li $t0 2
   sw $t0 -244($sp)

#------------392  FACTOR_CON CHAR 84 save in reg[61]
   li $t0 84
   sw $t0 -248($sp)

#------------393  ASSIGN_ARR_EXTERN CHAR testresult[reg[60]] = reg[61]
   lw $t0 -244($sp)
   lw $t1 -248($sp)
   sll $t0 $t0 2
   sw $t1 .var_testresult($t0)

#------------394  JUMP to label_36
   j label_36

#------------395  label_35    ----------------Label----------------
label_35:

#------------396  FACTOR_CON INT 2 save in reg[62]
   li $t0 2
   sw $t0 -252($sp)

#------------397  FACTOR_CON CHAR 70 save in reg[63]
   li $t0 70
   sw $t0 -256($sp)

#------------398  ASSIGN_ARR_EXTERN CHAR testresult[reg[62]] = reg[63]
   lw $t0 -252($sp)
   lw $t1 -256($sp)
   sll $t0 $t0 2
   sw $t1 .var_testresult($t0)
...
#------------401  FACTOR_FUNC INT testexpression save in reg[64]
   sw $v0 -260($sp)

#------------402  IF reg[64] BEQ reg[0] THEN GOTO label_37
   lw $t0 -260($sp)
    beq $t0 $0 label_37

#------------403  FACTOR_CON INT 3 save in reg[65]
   li $t0 3
   sw $t0 -264($sp)

#------------404  FACTOR_CON CHAR 84 save in reg[66]
   li $t0 84
   sw $t0 -268($sp)

#------------405  ASSIGN_ARR_EXTERN CHAR testresult[reg[65]] = reg[66]
   lw $t0 -264($sp)
   lw $t1 -268($sp)
   sll $t0 $t0 2
   sw $t1 .var_testresult($t0)

#------------406  JUMP to label_38
   j label_38

#------------407  label_37    ----------------Label----------------
label_37:

#------------408  FACTOR_CON INT 3 save in reg[67]
   li $t0 3
   sw $t0 -272($sp)

#------------409  FACTOR_CON CHAR 70 save in reg[68]
   li $t0 70
   sw $t0 -276($sp)

#------------410  ASSIGN_ARR_EXTERN CHAR testresult[reg[67]] = reg[68]
   lw $t0 -272($sp)
   lw $t1 -276($sp)
   sll $t0 $t0 2
   sw $t1 .var_testresult($t0)
...
#------------413  FACTOR_FUNC INT testarray save in reg[69]
   sw $v0 -280($sp)

#------------414  IF reg[69] BEQ reg[0] THEN GOTO label_39
   lw $t0 -280($sp)
    beq $t0 $0 label_39

#------------415  FACTOR_CON INT 4 save in reg[70]
   li $t0 4
   sw $t0 -284($sp)

#------------416  FACTOR_CON CHAR 84 save in reg[71]
   li $t0 84
   sw $t0 -288($sp)

#------------417  ASSIGN_ARR_EXTERN CHAR testresult[reg[70]] = reg[71]
   lw $t0 -284($sp)
   lw $t1 -288($sp)
   sll $t0 $t0 2
   sw $t1 .var_testresult($t0)

#------------418  JUMP to label_40
   j label_40

#------------419  label_39    ----------------Label----------------
label_39:

#------------420  FACTOR_CON INT 4 save in reg[72]
   li $t0 4
   sw $t0 -292($sp)

#------------421  FACTOR_CON CHAR 70 save in reg[73]
   li $t0 70
   sw $t0 -296($sp)

#------------422  ASSIGN_ARR_EXTERN CHAR testresult[reg[72]] = reg[73]
   lw $t0 -292($sp)
   lw $t1 -296($sp)
   sll $t0 $t0 2
   sw $t1 .var_testresult($t0)
...
#------------424  FACTOR_VAR INT flag save in reg[74]
   lw $t3 -4($sp)
   sw $t3 -300($sp)

#------------425  PUSH INT parareg[9] from reg[74]
   sw $ra -304($sp)
   lw $t0 -300($sp)
   sw $t0 -308($sp)

#------------426  CALL VOID testprintf(1)
   addi $sp $sp -308
   jal .func_testprintf
   subi $sp $sp -308
   lw $ra -304($sp)

#------------427  FACTOR_CON INT 0 save in reg[75]
   li $t0 0
   sw $t0 -304($sp)

#------------428  ASSIGN INT i = reg[75]
   lw $t3 -304($sp)
   sw $t3 0($sp)
...
#------------430  FACTOR_VAR INT i save in reg[76]
   lw $t3 0($sp)
   sw $t3 -308($sp)

#------------431  FACTOR_ARRAY_EXTERN CHAR testresult [reg[76]] save in reg[77]
   lw $t3 -308($sp)
   sll $t3 $t3 2
   lw $t3 .var_testresult($t3)
   sw $t3 -312($sp)

#------------432  WRITE CHAR reg[77]
   li $v0 11
   lw $a0 -312($sp)
   syscall
   li $v0 11
...
#------------433  FACTOR_VAR INT i save in reg[78]
   lw $t3 0($sp)
   sw $t3 -316($sp)

#------------434  FACTOR_CON INT 1 save in reg[79]
   li $t0 1
   sw $t0 -320($sp)

#------------435  reg[80] = reg[78] JIA reg[79]
   lw $t0 -316($sp)
   lw $t1 -320($sp)
   add $t2 $t0 $t1
   sw $t2 -324($sp)

#------------436  ASSIGN INT i = reg[80]
   lw $t3 -324($sp)
   sw $t3 0($sp)

#------------437  FACTOR_VAR INT i save in reg[81]
   lw $t3 0($sp)
   sw $t3 -328($sp)

#------------438  FACTOR_CON INT 5 save in reg[82]
   li $t0 5
   sw $t0 -332($sp)

#------------439  IF reg[81] BLSS reg[82] THEN GOTO label_41
   lw $t0 -328($sp)
   lw $t1 -332($sp)
    blt $t0 $t1 label_41
